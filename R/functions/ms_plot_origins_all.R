...
ms_plot_origins_all <- function(LocName,
                                     YearRange,
                                     DO_modelled_df, 
                                     DT_modelled, 
                                     n_origins = 20,
  ...
  MSType <- input$MSType_OriginsAll
  #"Total migrant stock (includes refugees)", "Non-refugees only", "Refugees only"
  ...
  if (!is.null(DO_modelled_df)) {
    ...
    if (MSType == "Non-refugees only") { 
      df$value <- df$value - df$refugees 
      DT_modelled$df$value <- DT_modelled$df$value - DT_modelled$df$refugees
    } 
    if (MSType == "Refugees only") { 
      df$value <- df$refugees 
      DT_modelled$df$value <- DT_modelled$df$refugees
    } 
    ...
    if (input$ChartType_OriginsAll == "Alluvial") {
     ...
     if (input$ChartType_OriginsAll == "Spaghetti") {
       
       p <- p +
         geom_line(data = df_unknown,
                     aes(x = year, y= unknown/1000), color = "darkgray") +
         geom_text(data = df_unknown %>% dplyr::filter(year == max(year)),
                   aes(x = year, y = unknown/1000), label = "Unspecified origin", color = "black", hjust = "left", size = 2.5)
       
       # Plot other light gray above unknown
       if (nrow(df_other[df_other$other != 0,]) >0) {
         p <- p +
           geom_line(data = df_other,
                       aes(x = year, y = other/1000), color = "lightgray") +
           geom_text(data = df_other %>% dplyr::filter(year == max(year)),
                     aes(x = year, y = other/1000), label = "Other known origins", color = "black", hjust = "left", size = 2.5)
         
       }
       # Then plot named origins
       for (i in 1:(length(origins))) {
         p <- p +
           geom_line(data = df_plot %>% dplyr::filter(origin == origins[i]),
                       aes(x = year, y = value/1000), color = colors[i]) +
           geom_point(data = df_plot %>% dplyr::filter(origin == origins[i]),
                      aes(x = year, y = value/1000,
                          text = paste(
                            ...
                            "MS:", sprintf("%.0f", round(value,0)), "\n")), color = colors[i], alpha = 0.1) +
           geom_text(data = df_plot %>% dplyr::filter(origin == origins[i] & year == max(year)),
                     aes(x = year, y = value/1000), label = origins[i], color = "black", hjust = "left", size = 2.5)
